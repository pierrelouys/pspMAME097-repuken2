...
#↓EBOOT.PBP 「実行圧縮」にしたいなら#を外してコンパイルしてね。
 COMPRESS_PBP = 1



# 実行圧縮 : PC でいう[UPX](ultimate packed exetracter)と同じ技術。
# 「メモリに圧縮されてる本体を自動で展開してから実行する」の略。タブン。

# EBOOT.PBP の本体プログラムを pspで実行直前に自動展開(gzip)する。
# その為 EBOOT.PBP のサイズが減らせる。(実行速度は変わらない)
# また「起動までの速度」も原理的には メモカ(Duo)が遅ければ遅い程、速くなる。
# ただし互換性はよくわからん。

# 元の 30% から 50% 程度に EBOOT.PBP が小さくなる。
...
$(PSP_EBOOT): $(FINAL_TARGET) $(PSP_EBOOT_SFO)
	$(STRIP) $(FINAL_TARGET) -o strip_prepacker_$(FINAL_TARGET)
	$(PSPPACK) strip_prepacker_$(FINAL_TARGET) -o strip_$(FINAL_TARGET)
	$(PACK_PBP) EBOOT.PBP $(PSP_EBOOT_SFO) $(PSP_EBOOT_ICON)  \
		$(PSP_EBOOT_ICON1) $(PSP_EBOOT_UNKPNG) $(PSP_EBOOT_PIC1)  \
		$(PSP_EBOOT_SND0)  strip_$(FINAL_TARGET) $(PSP_EBOOT_PSAR)
	$(RM) -f strip_prepacker_$(FINAL_TARGET)
