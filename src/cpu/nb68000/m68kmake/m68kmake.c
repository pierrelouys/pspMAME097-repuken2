...
under dos or windows 95/98/98SE. or XP compatibles mode.

quick start to dos prompt or dos.

you installed DOS/GCC/MINGW & typed the ...

DOS>gcc -O1 m68kmake.c -o m68kmake

then OK. maked 'm68kmake.exe'

// -O1			 : size optimizations.
// -mach=pentium : archtechture the upper compatibles for pentium or higher.
// m68kmake.c	 : current source file names.
// -o m68kmake	 : output object names the 'm68kmake.exe'

from (C)2006 PSP battle bursery scrool.

================================================
  battle nursery school  http://ameblo.jp/pspdevblog/	(home base)    (my blog)
  battle nursery school  http://psp.nukenin.jp/ 		(second base)  (home page at downloaded this file)
================================================
...
typedef struct
{
	...
	char mpu_mode/*[NUM_CPUS]*/;			/* User or supervisor mode */
	char mpus/*[1 NUM_CPUS+1]*/;			/* Allowed MPUs */
	char if_safety_dummy;
...
//int get_oper_cycles000(opcode_struct* op, int ea_mode);//, int cpu_type);
...
//static void write_prototype(FILE* filep, char* base_name);
...
void process_opcode_handlers(FILE* filep/*void*/);
...
//static FILE* g_ops_ac_file = NULL;
//static FILE* g_ops_dm_file = NULL;
//static FILE* g_ops_nz_file = NULL;
...
static ea_info_struct g_ea_info_table[13] =
{/* fname	 ea 	   mask  match */
...
static int g_size_select_table[33] =
{
	0,												/* unsized */
	0, 0, 0, 0, 0, 0, 0, 1, 						/*	  8    */
	0, 0, 0, 0, 0, 0, 0, 1, 						/*	 16    */
...
static int g_ea_cycle_table[13]/*[NUM_CPUS]*/[3] =
{/* 	  000			010 		  020			040  */
...
static int g_jmp_cycle_table[13] =
{
	 0, /* EA_MODE_NONE */
	 4, /* EA_MODE_AI	*/
	 0, /* EA_MODE_PI	*/
...
static int g_jsr_cycle_table[13] =
{
	 0, /* EA_MODE_NONE */
	 4, /* EA_MODE_AI	*/
	 0, /* EA_MODE_PI	*/
...
static int g_lea_cycle_table[13] =
{
	 0, /* EA_MODE_NONE */
	 4, /* EA_MODE_AI	*/
	 0, /* EA_MODE_PI	*/
...
static int g_pea_cycle_table[13] =
{
	 0, /* EA_MODE_NONE */
	 6, /* EA_MODE_AI	*/
	 0, /* EA_MODE_PI	*/
...
static int g_movem_cycle_table[13] =
{
	 0, /* EA_MODE_NONE */
	 0, /* EA_MODE_AI	*/
	 0, /* EA_MODE_PI	*/
...
static int g_moves_cycle_table[13][3] =
{
	{ 0,  0,  0}, /* EA_MODE_NONE */
	{ 0,  4,  6}, /* EA_MODE_AI   */
	{ 0,  4,  6}, /* EA_MODE_PI   */
...
static int g_clr_cycle_table[13][3] =
{
	{ 0,  0,  0}, /* EA_MODE_NONE */
	{ 0,  4,  6}, /* EA_MODE_AI   */
	{ 0,  4,  6}, /* EA_MODE_PI   */
...
//	if(cpu_type < CPU_TYPE_020)
	...
	//	{
	//		if(strcmp(op->name, "moves") == 0)	return op->cycles[cpu_type] + g_moves_cycle_table[ea_mode][size];
	//		if(strcmp(op->name, "clr") == 0)	return op->cycles[cpu_type] + g_clr_cycle_table[ea_mode][size];
	//	}
		...
		if(strcmp(op->name, "jmp") == 0)	return op->cycles/*[cpu_type]*/ + g_jmp_cycle_table[ea_mode];
		if(strcmp(op->name, "jsr") == 0)	return op->cycles/*[cpu_type]*/ + g_jsr_cycle_table[ea_mode];
		if(strcmp(op->name, "lea") == 0)	return op->cycles/*[cpu_type]*/ + g_lea_cycle_table[ea_mode];
		if(strcmp(op->name, "pea") == 0)	return op->cycles/*[cpu_type]*/ + g_pea_cycle_table[ea_mode];
		if(strcmp(op->name, "movem") == 0)	return op->cycles/*[cpu_type]*/ + g_movem_cycle_table[ea_mode];
	}
...
//	int i;
	...
	fprintf(filep, /*"}"*/"},\n");
#else
	fprintf(filep, "\t{%-28s, 0x%04x, 0x%04x, %3d},\n",
		op->name, op->op_mask, op->op_match, op->cycles );
#endif
}
void print_opcode_output_table(FILE* filep)
{
...
{
	*dst = *src;
	dst->cycles = get_oper_cycles000(dst, ea_mode);
...
//		if(output_file == g_ops_ac_file && oper_name[0] > 'c')		output_file = g_ops_dm_file;
//		else if(output_file == g_ops_dm_file && oper_name[0] > 'm') output_file = g_ops_nz_file;

		replace->length = 0;

		/* Generate opcode variants */
		if(strcmp(opinfo->name, "bcc") == 0 || strcmp(opinfo->name, "scc") == 0)
			generate_opcode_cc_variants(filep/*output_file*/, body, replace, opinfo, 1);
		else if(strcmp(opinfo->name, "dbcc") == 0)
			generate_opcode_cc_variants(filep/*output_file*/, body, replace, opinfo, 2);
		else if(strcmp(opinfo->name, "trapcc") == 0)
			generate_opcode_cc_variants(filep/*output_file*/, body, replace, opinfo, 4);
		else
			generate_opcode_ea_variants(filep/*output_file*/, body, replace, opinfo);
	}
...
int main(int argc, char **argv)
{
	...
	printf("\n\tMusashi 68000 emulator. 'M44+(v3)' @MAME106s_based. automaker. \n" );
	printf("\tCopyright 1998-2000 Karl Stenerud. (karl@mame.net)\n");
	printf("\tCopyright 2006-2007 PSP battle nursery school.\n\n");
	...
	if((g_prototype_file = fopen(filename, "wt")) == NULL)	perror_exit("Unable to create prototype file (%s)\n", filename);

	sprintf(filename, "%s%s", output_path, FILENAME_TABLE);
	if((g_table_file	= fopen(filename, "wt")) == NULL)	perror_exit("Unable to create table file (%s)\n", filename);

//	sprintf(filename, "%s%s", output_path, FILENAME_OPS_AC);
//	if((g_ops_ac_file	= fopen(filename, "wt")) == NULL)	perror_exit("Unable to create ops ac file (%s)\n", filename);
//
//	sprintf(filename, "%s%s", output_path, FILENAME_OPS_DM);
//	if((g_ops_dm_file	= fopen(filename, "wt")) == NULL)	perror_exit("Unable to create ops dm file (%s)\n", filename);
//
//	sprintf(filename, "%s%s", output_path, FILENAME_OPS_NZ);
//	if((g_ops_nz_file	= fopen(filename, "wt")) == NULL)	perror_exit("Unable to create ops nz file (%s)\n", filename);
...
//			fprintf(g_table_file, "%s", table_header_insert/*temp_insert*/);//moved to header inseart location.// ヘッダの出力位置変更mame117
			table_header_read = 1;
		}
		else if(strcmp(section_id, ID_OPHANDLER_HEADER) == 0)
		{
			...
			read_insert(ophandler_header_insert/*temp_insert*/);
		...
		else if(strcmp(section_id, ID_OPHANDLER_BODY) == 0)
		{
			...
			fprintf(g_table_file, "%s\n\n", ophandler_header_insert);//add... mame117-
			process_opcode_handlers(g_table_file);
			fprintf(g_table_file, "%s\n\n", ophandler_footer_insert);//add... mame117-
		...
		else if(strcmp(section_id, ID_END) == 0)
		{
			...
			if(!prototype_header_read)	error_exit("Missing prototype header");
			if(!prototype_footer_read)	error_exit("Missing prototype footer");
			if(!table_header_read)		error_exit("Missing table header");
			if(!table_footer_read)		error_exit("Missing table footer");
			if(!table_body_read)		error_exit("Missing table body");
			if(!ophandler_header_read)	error_exit("Missing opcode handler header");
			...
			fprintf(g_table_file, "%s", table_header_insert/*temp_insert*/);// moved from header inseart location.// ヘッダの出力位置変更mame117
