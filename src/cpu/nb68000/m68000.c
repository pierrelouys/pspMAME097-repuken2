...
//#define USE_GOTO 1
  ...
  #define USE_TEMP_TYPE 0/* musashi compatible(use per subroutine) */
//#define USE_TEMP_TYPE 1/* use by structure */
//#define USE_TEMP_TYPE 2/* use common one at static in obj */


/* temp Opecode */
//#define USE_TEMP_IR_TYPE 0/*dummy*/
//#define USE_TEMP_IR_TYPE 1/* musashi compatible(use by structure) */
  #define USE_TEMP_IR_TYPE 2/* use common one at static in obj */


  #define USE_LOCALITY 0
//#define USE_LOCALITY 1

#if (0==USE_TEMP_TYPE)
	#define TEMP_UINT32 uint
#else
	#define TEMP_UINT32 /**/
#endif // (0==USE_TEMP_TYPE)


#if (1==USE_GOTO)
	#define RET(aaa) {m68ki_initial_cycles -= aaa; goto C68k_Exec_Next;}
	#define OP(aaa) aaa:
#else
	#define RET(aaa) return;
	#define RET_0_   return;
	#if (1==USE_LOCALITY)
		#define OP(aaa) static void aaa(m68ki_cpu_core *CPU)
	#else
		#define OP(aaa) static void aaa(void)
	#endif
#endif //(1==USE_GOTO)

#if (1==USE_LOCALITY)
	#define M68KI_CPU_PTR CPU->
#else
	#define M68KI_CPU_PTR m68ki_cpu.
#endif
...
static /*int*/short  m68ki_remaining_cycles = 0;					 /* Number of clocks remaining */
#else
...
static UINT8 m68ki_shift_8_table[65-1] =
{
...
static UINT16 m68ki_shift_16_table[65-1] =
{
...
static UINT32 m68ki_shift_32_table[65-1] =
{
 ...
 */
static const UINT8 m68ki_exception_cycle_table000[48/*256*/] =
{
//	{ /* 000 */
...
static const UINT8 m68ki_ea_idx_cycle_table[64] =
{
...
#if 0
//(m68010+)void m68k_set_bkpt_ack_callback(void  (*callback)(unsigned int data))
//{
...
int m68k_execute(int num_cycles)
{
	...
	if(!MPU_STOPPED)
	{
		...
		/* ASG: update cycles */
//		USE_CYCLES(CPU_INT_CYCLES);	CPU_INT_CYCLES = 0;
...
//(m68010+)	m68k_set_bkpt_ack_callback(NULL);
#if (1==M68K_USE_SYS16_CALLBACK)
...
//	REG_VBR = 0;
	...
	//INLINE void m68ki_set_sm_flag(uint value)
	{
		...
		FLAG_S = SFLAG_SET;///*value*/(((value >> 11) /*& 6*/)) & SFLAG_SET/*4*/;
		FLAG_M = MFLAG_CLEAR;///*value*/(((value >> 11) /*& 6*/)) & MFLAG_SET/*2*/;
...
unsigned int m68k_get_context(void* arg_dst)
{
	if(arg_dst) *(m68ki_cpu_core*)arg_dst = m68ki_cpu;
	return sizeof(m68ki_cpu_core);
}

void m68k_set_context(void* arg_src)
{
	if(arg_src) m68ki_cpu = *(m68ki_cpu_core*)arg_src;
}
