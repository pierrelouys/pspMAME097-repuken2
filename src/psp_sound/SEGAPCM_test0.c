...
{
/* machine->drv->sound[1].configが 1 なのは、
	0(つまり始めに登録) が YM2151 で、
	1(つまり２番目に登録) がsegaPCMだから。
	詳しくは src/drivers/outrun.c を参照せよ。 */
...
//	spcm->rate = /*SEGAPCM_SAMPLE15K*/external_clock/*external_clock*/;
	...
	/* rom addr */
	spcm->rom = (const UINT8 *)memory_region(intf->region); 			/* ちなみに src/drivers/outrun.c の場合 REGION_SOUND1 */
	spcm->rom_end = spcm->rom + memory_region_length(intf->region); 	/* ちなみに src/drivers/outrun.c の場合 REGION_SOUND1 */
	...
	/*	*/
	spcm->bankshift = (UINT8)(intf->bank);	/* ちなみに src/drivers/outrun.c の場合 BANK_512 */

	/*	*/
	int mask;
	mask = intf->bank >> 16;				/* ちなみに src/drivers/outrun.c の場合 BANK_512 */
