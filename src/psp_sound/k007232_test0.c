...
//	unsigned int  clock;		  /* chip clock */
	...
	INT32 buffer[PSP_SOUND_SAMPLES];//	sound_stream * stream;

	const struct K007232_interface *intf;
	float fncode[0x200];
} KDAC_A_PCM;

static struct kdacApcm *my7232_chip[2];/* K007232 a-jax have 2 chips. */
...
static void KDAC_A_update(int chip_num,/*void *param,*/ /*stream_sample_t*//*INT16 **inputs,*/
/*stream_sample_t*//*INT16 **buffer*//*INT32 *buf,*/ int buffer_len)
{
	struct kdacApcm *info_aaa=my7232_chip[0];
INT32 *buf;

	struct kdacApcm *info = my7232_chip[chip_num]/*sndti_token(SOUND_K007232, chip)*/;
...
//	memset(buf, 0, (PSP_SOUND_SAMPLES-1)/*(length+length)*//*sizeof(buf)*/);
//	INT32 *buf = t_buffer;

	for( i = 0; i < KDAC_A_PCM_MAX; i++ )
	{
	buf=&(info_aaa->buffer[0]);
		if (info->play[i])
		{
		int volA,volB,j,out;
		...
		//int cen;
			...
			volA = info->vol[i][0] /* * 2*/;/*monoral*/
			volA -= (volA>>4);/* 6.25%(=:=5%) loss */
			volB = info->vol[i][1] /* * 2*/;/*monoral*/
			volB -= (volB>>4);/* 6.25%(=:=5%) loss */
#if 0
			 cen = (volA + volB) / 2;
...
#if 0
/*stereo*/
				/*buffer[0][j]*//*(*buf++)*/mixing_buffer[jjj] =  += out * volA *2;
				/*buffer[1][j]*//*(*buf++)*/mixing_buffer[jjj] =  += out * volB *2;
#else
/*monoral*/
		//		/*buffer[0][j]*//*(*buf++)*/(*buf) += (INT32)(out * (volA) );
		//		/*buffer[0][j]*//*(*buf++)*/(*buf) += (INT32)(out * (volB) );
				/*buffer[0][j]*//*(*buf++)*/(*buf) += (INT32)(out * ((volA+volB)/2) );/*48%*/
				buf++;
#endif
			}
		}
...
{
	struct kdacApcm *info = my7232_chip[chip]/*sndti_token(SOUND_K007232, chip)*/;
...
//	//stream_update(info->stream,0);
	...
	//	if( !reg_port && r == 1 )	logerror("%04x\n" ,data );
		info->step[0/*reg_port*/] = info->fncode[data] * (float)info->ppp;
		break;
//	case 0x02:
//	case 0x03:
//	case 0x04:	break;
	case 0x05:
		...
		info->start[0/*reg_port*/] =
			((((unsigned int)info->wreg[/*reg_port*0x06 +*/ 0x04]<<16)&0x00010000) |
			 ...
			 info->bank[0/*reg_port*/]);
		if (info->start[0/*reg_port*/] < info->pcmlimit ){
			info->play[0/*reg_port*/] = 1;
			info->addr[0/*reg_port*/] = 0;
		}
		break;
	case (0x00+6):
	case (0x01+6):
		/**** address step ****/
		data = (((((unsigned int)info->wreg[/*reg_port*/1*0x06 + 0x01])<<8)&0x0100) | (((unsigned int)info->wreg[/*reg_port*/1*0x06 + 0x00])&0x00ff));
	//	if( !/*reg_port*/1 && r == 1 )	logerror("%04x\n" ,data );
		info->step[/*reg_port*/1] = info->fncode[data] * (float)info->ppp;
		break;
	case (0x05+6):
		/**** start address ****/
		info->start[/*reg_port*/1] =
			((((unsigned int)info->wreg[/*reg_port*/1*0x06 + 0x04]<<16)&0x00010000) |
			 (((unsigned int)info->wreg[/*reg_port*/1*0x06 + 0x03]<< 8)&0x0000ff00) |
			 (((unsigned int)info->wreg[/*reg_port*/1*0x06 + 0x02]	 )&0x000000ff) |
			 info->bank[/*reg_port*/1]);
		if (info->start[/*reg_port*/1] < info->pcmlimit ){
			info->play[/*reg_port*/1] = 1;
			info->addr[/*reg_port*/1] = 0;
		}
		break;
	case 0x0c:
...
//	struct kdacApcm *info = sndti_token(SOUND_K007232, chip);
	...
	if( r == 0x0005 || r == 0x000b ){
		ch = ((r>>3)&1);//(r==5)?0:1;//r/0x0006;
		r-=5;//	r  = (r==5)?0:6;//ch * 0x0006;
...
/*****************************************************************************/
  ...
  WRITE8_HANDLER( K007232_write_port_0_w ){	K007232_WriteReg(offset,data,0);}
  READ8_HANDLER(  K007232_read_port_0_r ){	return K007232_ReadReg(offset,0);}
#ifdef ajax
WRITE8_HANDLER( K007232_write_port_1_w ){	K007232_WriteReg(offset,data,1);}
READ8_HANDLER(  K007232_read_port_1_r ){	return K007232_ReadReg(offset,1);}
#endif
//WRITE8_HANDLER( K007232_write_port_2_w ){	K007232_WriteReg(offset,data,2);}
//READ8_HANDLER(  K007232_read_port_2_r ){	return K007232_ReadReg(offset,2);}
...
//	for( i = 0; i < 0x200; i++ )	logerror("fncode[%04x] = %.2f\n", i, fncode[i] );
...
/*	  Konami PCM start							*/
/************************************************/
static void k007232_start(/*int*//*UINT8 sndindex,*/ int clock/*, const void *config*/,int chip_num)
{
	int i;
...
//
	struct K007232_interface *config;
#define REGIST_ON_DRIVER_THE_HEAD_NUMBER (1)
config = machine->drv->sound[REGIST_ON_DRIVER_THE_HEAD_NUMBER+chip_num].config;// config;
#undef	REGIST_ON_DRIVER_THE_HEAD_NUMBER
	info->intf = config;
//	/*info->*/intf = config;
