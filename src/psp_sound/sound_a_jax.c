...
#ifndef _A_JAX_SOUND_H_
#define _A_JAX_SOUND_H_

/*****************************************************************************
*
*	konami A-JAX sound driver (YM2151 + K007232 + K007232)
*
******************************************************************************/
...
//	if (n63701x->rom) n63701x_update(length);
	...
	memset(info_aaa->buffer, 0, sizeof(info_aaa->buffer));
	KDAC_A_update(0,/*kdacApcm*//* &info_aaa->buffer[0],*/ length);
	KDAC_A_update(1,/*kdacApcm*//* &info_aaa->buffer[0],*/ length);
	#if (0==NO_SEGAPCM)
	//SEGAPCM_Update_stream(length);
	#endif //(0==NO_SEGAPCM)
//	namco_cus30_update(length);
//	if (n63701x->rom) n63701x_update(length);
	...
	//	out_MONO  = namcos1->buffer[i];
	//	out_MONO += n63701x->buffer[i];
		out_MONO += ((info_aaa->buffer[i]) );
	//	out_MONO += ((info_bbb->buffer[i])/2);/*48%*/
...
//	YM2151Init(3579545, samplerate, NULL);
	YM2151Init(3579545, samplerate, NULL);// FZもoutrunも4[MHz]
#if 0
...
#endif //(0==NO_SEGAPCM)
#if 1
	k007232_start(3579545, 0);
	k007232_start(3579545, 1);
#endif
...
//	switch(sndnum)
//	{
//	case 0: return 3579545; /* 4.000[MHz] */  // FZもoutrunも4[MHz]
...
//	}
//	return 0;
	return 3579545;	/* 3.579545[Hz] (?)  */ /* サンダークロスでおかしくなる(?) */
//	return 3570000;	/* 3.570000[Hz] (?)  */ /* サンダークロスで正常(?) */
}
const char *sndnum_get_info_string(int sndnum, UINT32 state)
{
	...
	case 1: return "K007232";	// outrunのみ.
